...
class NewsViewController: UIViewController {
    ...
    @IBOutlet weak var featuredNewsCollectionView: UICollectionView! {
        didSet {
            ...
            self.featuredNewsCollectionView.register(UINib(nibName: String(describing: NewsCollectionViewCell.self), bundle: nil),
...
extension NewsViewController: UICollectionViewDataSource, UICollectionViewDelegate, UIScrollViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        if scrollView == self.featuredNewsCollectionView {
            ...
            let enoughVelocityToGoToPreviousCell = indexOfCellBeforeDragging - 1 >= 0 && velocity.x < -swipeVelocityThreshold
            let majorCellIsTheCellBeforeDragging = indexOfMajorCell == indexOfCellBeforeDragging
            let useSwipeToSkipCell = majorCellIsTheCellBeforeDragging && (hasEnoughVelocityToSlideToTheNextCell || enoughVelocityToGoToPreviousCell)
            if useSwipeToSkipCell {
                let indexPath = IndexPath(row: indexOfCellBeforeDragging, section: 0)
                self.featuredNewsCollectionView.collectionViewLayout
                    .collectionView!
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: String(describing: NewsCollectionViewCell.self),
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let viewController = NewsResultViewController()
        viewController.newsViewModel = viewModel.modelResult?.articles[indexPath.row]
