...
extension SearchHospitalsResultViewController {
    ...
    func distanceBeteweenKm(latitude: String, longitude: String) {
        let coordinate₀ = CLLocation(latitude: (Double(latitude))!,
                                     longitude: (Double(longitude))!)

        let coordinate₁ = CLLocation(latitude: (Double(self.managerLatitude ?? ""))!,
                                     longitude: (Double(self.managerLongitude ?? ""))!)
...
extension SearchHospitalsResultViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "celula",
                                                      for: indexPath) as? SearchHospitalsResultTableViewCell
